...
import arc.collection.*;
import arc.collection.ObjectMap.Entry;
import arc.collection.Queue;
import arc.collection.OrderedMap.OrderedMapValues;
...
public class Json{
    ...
    public void writeValue(Object value, Class knownType, Class elementType){
        try{
            ...
            if(value instanceof arc.collection.Queue){
                if(knownType != null && actualType != knownType && actualType != arc.collection.Queue.class)
                    throw new SerializationException("Serialization of a Queue other than the known type is not supported.\n"
                    + "Known type: " + knownType + "\nActual type: " + actualType);
                writeArrayStart();
                arc.collection.Queue queue = (arc.collection.Queue)value;
    ...
    public <T> T readValue(Class<T> type, Class elementType, JsonValue jsonData){
        ...
        if(jsonData.isArray()){
            ...
            if(ClassReflection.isAssignableFrom(arc.collection.Queue.class, type)){
                arc.collection.Queue result = type == arc.collection.Queue.class ? new arc.collection.Queue() : (Queue)newInstance(type);
                for(JsonValue child = jsonData.child; child != null; child = child.next)
